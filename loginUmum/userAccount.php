...
if(isset($_POST['signupSubmit'])){
    ...
    if(!empty($_POST['first_name']) && !empty($_POST['last_name']) && !empty($_POST['email']) && !empty($_POST['phone']) && !empty($_POST['password']) && !empty($_POST['confirm_password'])){
        ...
        if($_POST['password'] !== $_POST['confirm_password']){
            ...
            $sessData['status']['msg'] = 'sandi konfirmasi harus sama dengan sandi'; 
        }else{
            ...
            if($prevUser > 0){
                ...
                if($insert){
                    ...
                    $sessData['status']['msg'] = 'anda telah terdaftar, silahkan login';
                }else{
                    $sessData['status']['type'] = 'error';
        ...
        $sessData['status']['type'] = 'error';
        $sessData['status']['msg'] = 'mohon isi semua bidang'; 
    }
    ...
    if(!empty($_POST['email']) && !empty($_POST['password'])){
        ...
        $sessData['status']['type'] = 'error';
        $sessData['status']['msg'] = 'masukan email atau sandi'; 
    }
    ...
    $sessData['status']['type'] = 'success';
    $sessData['status']['msg'] = 'anda telah keluar logout dari akun';
    $_SESSION['sessData'] = $sessData;
    //redirect to the home page
    header("Location:index.php");
}else{
