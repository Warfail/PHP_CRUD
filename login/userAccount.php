...
if(isset($_POST['signupSubmit'])){
    ...
    if(!empty($_POST['first_name']) && !empty($_POST['last_name']) && !empty($_POST['email']) && !empty($_POST['phone']) && !empty($_POST['password']) && !empty($_POST['confirm_password'])){
        ...
        if($_POST['password'] !== $_POST['confirm_password']){
            ...
            $sessData['status']['msg'] = 'sandi harus sama!.'; 
        }else{
            ...
            if($prevUser > 0){
                $sessData['status']['type'] = 'error';
                $sessData['status']['msg'] = 'email sudah digunakan';
            }else{
                ...
                if($insert){
                    ...
                    $sessData['status']['msg'] = 'anda telah terdaftar silahkan login dengan data yang anda masukan';
                }else{
                    $sessData['status']['type'] = 'error';
        ...
        $sessData['status']['type'] = 'error';
        $sessData['status']['msg'] = 'mohon isi semua'; 
    }
